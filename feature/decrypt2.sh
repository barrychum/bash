...
HASH_LENGTH=32          # MD5 hash is 32 characters
ENC_SYM_KEY_LENGTH=256  # Assuming 2048-bit RSA key, adjust as necessary

# Extract the encrypted symmetric key and hash from the bundle
{
  # Extract the encrypted symmetric key
  ENC_SYM_KEY=$(head -c $ENC_SYM_KEY_LENGTH | xxd -p)
  
  # Extract the hash
  PUBLIC_KEY_HASH=$(tail -c +$(($ENC_SYM_KEY_LENGTH + 1)) | head -c $HASH_LENGTH)
  
  # Extract the encrypted file content
  tail -c +$(($ENC_SYM_KEY_LENGTH + $HASH_LENGTH + 1)) > "$ENCRYPTED_FILE"
} < "$ENCRYPTED_BUNDLE"
